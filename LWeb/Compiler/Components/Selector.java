...
import LWeb.Compiler.Components.Attribute.AttributeStyle;
...
import static LWeb.Compiler.Components.Selector.Combinators.DESCENDANT;
import LWeb.Common.Tree.Node;
import static LWeb.Compiler.Components.Selector.Combinators.SELF;
...
public class Selector {
    ...
    *///                        " "       ">"   "+"    "~"
    public enum Combinators{DESCENDANT, CHILD, NEXT, SUBSEQUENT, SELF}
    ...
    private boolean subValCap(Node<ElementTag> ell, int index){//doa in reverse
        ...
        switch(cm){
            ...
            case NEXT:
                ...
                if(nxb) nxb = subValCap(node_nx, index-1);
                return nxb;
            case SUBSEQUENT:
                ...
                do{
                    ...
                    if(acc_sq)acc_sq=subValCap(node_sq, index-1);
                    j--;
                }while(!acc_sq);
                return true;
        }
    ...
    public static class CondClass implements Conditions{
        final ClassList cls;
        public CondClass(ClassList cls){
            this.cls=cls;
        }
        public CondClass(String cls){
            this.cls=ClassList.getClass(cls);
        }
        public boolean validate(Node<ElementTag> ell) {
            Activity cs = ell.data.type(cls);
    ...
    //may not work for ell.cls1:not(.cls2)
